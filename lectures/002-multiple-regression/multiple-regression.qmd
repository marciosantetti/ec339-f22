...
Let's look at an example with $k=2$ independent variables:
...
The estimated value of $\hat{\beta}_1$ is -0.88. This means that, holding an individual's disposable income constant, a one-dollar increase in the price of beef *decreases* beef consumption by .88 pounds. Likewise, $\hat{\beta}_2$ is 11.9, meaning that, *ceteris paribus* (i.e., holding the price of beef constant), if an individual's disposable income icreases by one thousand dollars, her beef consumption will increase by 11.9 pounds. Notice that we have to respect the **measurement units** by which the variables are defined here, adapting the 1-unit change to the respective way in which they are measured.
...
After our regression is estimated, we would like to assess *how well* our model fits the data. You may have been introduced introduced to the *coefficient of determination*, $R^2$, elsewhere, and it assesses the **variation** in $y$ caused by **variations** in our independent variable(s). And you have probably learned that it is calculated by squaring the correlation coefficient, usually known as $r$, thus ranging between 0 and 1, or 0 and 100\%.
...
In case you chose "Case 2," you live in the real world. In case we lived in the world of "Case 1," there would be no uncertainty: just stay on the job for the rest of your life, and your wage will increase no matter what. We know that this is basically impossible. 
...
What we expect is that one's salary at least increases by a **constant percentage**, the more educated she becomes. Graphically, it looks somewhat like the graph below, relating two variables, $x$ and $y$.
...
Depending on our interpretation purposes, we can also log-transform the the independent variables of our model. If we decide to keep $y$ in *log*-form, then we have a **log-log** functional form. This setting is known as *constant elasticity models*. Let's look at an example:

$$ log(salary_i) = \beta_0 + \beta_1log(sales_i) + u_i  $$

Since both the dependent and independent variable are in *logs*, we can interpret $\hat{\beta}_1$ as the **elasticity of salary with respect to sales**. You must recall the concept of *elasticity* from your Microeconomics classes; if not, make sure to give it a quick review.
