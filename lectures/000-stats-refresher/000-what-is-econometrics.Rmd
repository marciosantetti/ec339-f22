...
xaringanExtra::use_editable(expires = 2)
```
...
.hi[Cross-section data]:

  - Data on one or more variables, collected at the same point in time, for different or the same individuals (*i*). These individuals may be persons, states, countries, plants, firms, etc.
    - Census;
    - Opinion polls.
...
.hi[Time series data]:
  ...
  - Set of observations on the value that a variable takes at different times (daily, monthly, quarterly, annually, etc.)
    - Daily: stock prices; 
    - Monthly: CPI, unemployment rate;
    - Quarterly: GDP;
    - Annually: Government budgets.
...
.hi[Panel (longitudinal) data]:

  * Same (or different) cross-sectional units (*i*) is (are) surveyed over time (periodically).
      * Example: Housing Census by the US Department of Commerce.

---
layout: false
class: inverse, middle

# Next time: Stats/Stata refresher
