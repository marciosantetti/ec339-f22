...
The average marginal effect of *income* on *part* is -.2203. This means that if an individual's income increases by 1\%, she is 22.03\% less likely to join the labor force, *ceteris paribus*.^[ For this data set, income is log-transformed.]
...
Holding all other factors constant, a 1\% increase in an individual's income decreases the likelihood of participating in the labor force by 22.09\%. The Probit estimation returns a similar result as the one seen for the Logit estimation, and the next figure illustrates its curve.
