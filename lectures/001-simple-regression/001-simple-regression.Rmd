...
Life expectancy _vs._ GDP per capita (1952--2007):<sup>*<sup>
...
p + geom_point(aes(color = continent), alpha=0.7, size = 2) +
...
Now, including .hi[regression lines]:

```{r, echo=F, message=F, warning=F, dev = "svg", fig.height = 5}


p + geom_point(aes(color = continent), alpha=0.4, size = 2) + 
...
gapminder %>% filter(continent == 'Americas') %>% 
  ggplot(aes(x=gdpPercap, y=lifeExp)) + scale_x_log10() + geom_point(aes(color=country), alpha=0.4, size = 2) + 
...
# Next time: Simple regression in .mono[Stata]
